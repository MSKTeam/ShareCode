const nPLPro = {
	...
	create: function(t, e = []) {
		Promise.all(e.map(e => {
			let n = `/feeds/posts/summary/-/${t}?alt=json-in-script&start-index=${e}&max-results=150`;
			return jQuery.ajax({
				type: "get",
				url: n,
				dataType: "jsonp"
			...
			t.forEach(t => {
				"entry" in t.feed && t.feed.entry.forEach(t => {
					let e = t.link.find(t => "alternate" === t.rel).href.split("?")[0];
					this.arr.push({
						title: t.title.$t,
						link: e
					})
				})
			}), this.arr.length > 0 ? this.compile() : console.log("Error: no entries found.")
